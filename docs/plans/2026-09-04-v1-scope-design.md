...
- **Ephemeral Session Mode.** Temporary installs on untrusted hosts (an internet cafe laptop, a shared workstation) need a session-scoped lifecycle: the operator uses holzi normally during the session, then on explicit session end (or on process exit, if configured) the local haex-crdt store, per-device keypairs, and any cached federation state are wiped so nothing remains on the host. This overlaps with but is not solved by the empty-capability-grant flow in Section 4: even without either slot, the device still writes an attestation into CRDT-synced state and holds a device keypair on disk. Open sub-questions: does the temporary device write its attestation into shared CRDT state at all, or does it remain an unattested "guest peer" signed by an already-attested parent for the session's duration only? Where is the boundary between "attested with no capabilities" and "unattested guest participant"? Whether Ephemeral Session Mode is a v1 or post-v1 feature is not decided here.
