...
- **Genesis moment.** On first install, a holzi instance can start a fresh federation. It generates a federation-root keypair in-app and displays a **paper-seed** derived from that keypair. The operator is required to record and store the paper-seed before the setup completes; the seed is the sole federation-scope recovery secret and is the only artifact that ever leaves the federation's encrypted state boundary.
- **Symmetric pairing.** Any already-attested device may pair a new device. Pairing runs by short-lived one-time token (QR code or copy/paste string) issued by the parent device and scanned by the joiner. The parent signs an attestation for the joiner as part of the ceremony. After pairing, the joiner is a full peer with pairing authority of its own.
- **CRDT-synced attestation ring.** The federation state (paper-seed-derived signing material, attested-device registry, revocation epochs, capability grants including `confirmation-authority` and `pairing-authority`) lives in the encrypted CRDT store described in Section 5. Every attested device therefore holds a live copy of the federation's signing capability.
- **Compromise model.** Because every attested device holds signing material, compromise of any one device compromises the federation's ability to issue new attestations. Response is fast revocation via epoch bump (as in `founding.md`), initiated from any surviving device. The paper-seed remains untouched by device compromise, since it is not stored on any device.
- **Recovery.**
...
- **What remains from `founding.md`.** The trust-store shape (`alias, nostr_pubkey, iroh_node_id, valid_until, capabilities, epoch`) is unchanged. The epoch-bump revocation semantics are unchanged. The `attestation` event and revocation event are unchanged in shape; what changes is *who signs them* — any attested device with `pairing-authority`, not an external master.
- **Attestation-capability slots.** `pairing-authority` and `confirmation-authority` are attestation-capability slots, delegable and revocable through the same attestation flow. Genesis-device holds both by default. Whether they always co-travel or can be split (a "confirmer" device that cannot pair, or a "pairer" device that cannot confirm writes) is spec-phase work.
...
- **What lives in `haex-crdt`-synced state.** Federation-scope encrypted state that must be identical across attested devices: paper-seed-derived signing material, attested-device registry, revocation-epoch table, capability grants, chat/session history, skills/memory (post-v1 scope but reserved).
...
- **Three cross-device channels, disjoint by role.** With `haex-crdt` in play, holzi has three cross-device channels rather than the two in `founding.md`:
  ...
  These channels do not share content. Nostr never carries CRDT deltas; `haex-crdt` never carries a command intent; iroh never carries state. Which transport `haex-crdt` uses internally (its own WS bridge, iroh, or something else) is a `haex-crdt`-internal decision, not a holzi-visible one.
...
- **Presence events carry availability class.** Each device's presence event declares an availability class — working proposals: `always-on` (typical desktop or server), `foreground-only` (typical mobile). Availability class is normative for sender behavior: a command targeted at a `foreground-only` device does not wait on long relay timeouts; it fails fast with an "unreachable" result and can be retried later.
- **Delivery semantics for mobile targets.** Command intents addressed to a currently-unreachable `foreground-only` device: **queued at the sender's home relay with a TTL**, delivered when the target next publishes a fresh presence event, dropped when TTL expires. Exact TTL, delivery-once semantics, and the presence-event kind carrying availability class are spec-phase work.
...
1. Holzi Tauri app boots on device A. First-run wizard generates a fresh federation-root keypair inside `haex-crdt`-encrypted state, derives and displays the paper-seed, requires operator to confirm they have recorded it.
2. Device A generates a device-scoped Nostr keypair and iroh NodeId; both are attested against the federation root via a symmetric-cross-signing attestation event stored in `haex-crdt`.
...
5. Device B generates its own Nostr/iroh identities; Device A signs a joiner-attestation for Device B; `haex-crdt` synchronizes the attestation ring between the two devices.
...
- **From this document, still open:** presence-event kind and availability-class wire format; sender queue TTL for `foreground-only` targets; embedded-runner stack selection (llama.cpp vs candle vs mistral.rs); exact `haex-crdt` crate boundaries and its own transport; whether `pairing-authority` and `confirmation-authority` can be split.
...
1. Amend `founding.md` with a top-of-file note pointing here.
2. Wait on haex-hive migration and `haex-crdt` extraction.
3. Start the "Project structure and spec workflow" question (`founding.md` §5 item 1): whether to adopt speckit as haex-hive does, adopt it lightly, or use a different approach. This can begin before the two blockers clear.
