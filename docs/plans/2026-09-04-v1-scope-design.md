...
- **Device identity for HLC (`DeviceIdProvider`).** Decided 2026-09-07. holzi mints device UUIDs in
  ...
  Cross-host device handover is **out of v1**: the pinned haex-crdt revision has no
  `DeviceIdPolicy`/`AdoptOnMismatch` API, and Holzi MUST NOT pretend that a moved database can be
  opened by supplying a new UUID. A future contract revision may add that policy after pinning a
  released haex-crdt revision that supports it. Until then the local device-ID index is the
  source for ordinary reopens and same-host imports; a missing entry is an explicit validation
  failure, not permission to mint a replacement UUID.
