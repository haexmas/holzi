...
- **Instance = SQLite = identity.** Each SQLite database is an independent instance. On instance creation, holzi generates a NIP-01-compatible `secp256k1` keypair for Nostr event signing and a separate Ed25519 iroh NodeId keypair, and stores both inside the database (in a holzi-owned `instance_identity_no_sync` table). The active SQLite is the operator's federation-facing identity for that session; opening a different SQLite switches identity. Keys never leave the encrypted database.
- **No federation-root, no paper-seed.** There is no persistent federation-scope keypair in v1. "The federation" is the mutual-trust graph formed by CRDT-synced peer records; it has no cryptographic root. The only secret the operator handles explicitly is the SQLCipher passphrase per database — the same one they use to unlock the app. Recovery is by a surviving synced instance. Adopting a copied `.db` is an intended replica-adding path but is blocked until a haex-crdt revision with an adoption API is reviewed and pinned. This applies to every imported copy, even when the source UUID is in the local index: the source may still run, so the copy needs a distinct CRDT node identity. Ordinary reopen of the original managed database retains its UUID; staging an import does not make it usable. Every restore MUST use rekey-on-restore, regardless of which machine the file came from: before starting any relay or iroh endpoint, holzi prepares fresh Nostr `secp256k1` and iroh Ed25519 keypairs, signs the handover payload while the copied Nostr key is usable, and writes that payload before atomically replacing the singleton local identity row and retiring the copied identity. The attestation, matching fresh private keys and local completion proof commit together; the explicit token fallback records its completion without a grant when no valid attestation can be produced. Key rotation alone is insufficient: adoption must also mint a fresh device UUID, rotate the CRDT signing key, and durably coordinate the target index and restore markers before application writes or endpoint startup. A retry must preserve an already adopted identity. Authorization of the adopted replica comes from a **handover attestation** binding the final UUID, signing/endpoint public keys and authorization fields, signed with the copied Nostr key before it is retired. With a locally valid attestation, adoption completes offline without a reachable parent (operator decision, 2026-09-08); token pairing remains the fallback when no valid attestation can be produced. Handover grants obey the same current-source `pairing-authority`, capability-subset, expiry and epoch checks as other peer grants; a local offline snapshot cannot guarantee acceptance against a receiver's newer revocation state. The source instance may remain active without creating duplicate live identities. All-devices-lost with no backup means data-lost; that is an accepted risk that the paper-seed did not actually mitigate (it would have restored an empty federation identity).
...
- **Federation-wide reset (panic button).** Response to a compromise the operator cannot outrun with individual revocations. Every surviving trusted instance atomically replaces the singleton `instance_identity_no_sync` row in a local transaction: retire and remove the old private keys, generate and persist the fresh Nostr `secp256k1` and iroh Ed25519 keypairs in that same row, and only then start endpoints or re-pair. No second identity row is inserted. All existing `peer_instances` records are declared invalid at a cutoff epoch communicated out-of-band, and instances re-pair against the fresh identities. State to preserve (chat history, skills, memory) is exported from a trusted `.db` snapshot before reset and re-imported into the freshly-paired federation. There is no new federation-root to generate because there is no federation-root; the reset is simply "everyone rolls their instance keys and re-pairs".
...
- **What lives in `haex-crdt`-synced state.** Federation-scope encrypted state that must be identical across paired instances: `peer_instances` registry, revocation-epoch table, capability grants (per-peer flags), chat/session history, skills/memory (post-v1 scope but reserved). Per the 2026-09-06 §4 revision there is no capability-scoped signing material to replicate — instances only ever sign with their own keypair from `instance_identity_no_sync`, which is a local-only record and MUST NOT sync across peers.
- **What does not live in `haex-crdt`.** Ephemeral runtime state (open iroh sessions, current relay connections), local-only preferences that should not sync (device alias, local model file paths), and the per-instance secret keys in `instance_identity_no_sync` (Nostr keypair + iroh NodeId keypair) that live inside the SQLite but MUST be excluded from CRDT delta transmission — they are per-instance-database secrets, not federation-shared state.
- **Device identity for HLC (`DeviceIdProvider`).** Decided 2026-09-07. holzi mints device UUIDs in
  ...
  Adopting a copied database on this or another machine is an **intended way to add a replica**,
  alongside token/QR pairing — the operator picks whichever fits the situation. It is **blocked
  today** because the pinned haex-crdt revision has no `AdoptOnMismatch` API, and Holzi MUST NOT
  pretend that a moved database can be opened merely by supplying a new UUID. Until that API
  lands, the local device-ID index is the source for ordinary reopens, and a missing entry is an
  explicit validation failure rather than permission to mint a replacement UUID. Import-pending
  copies with known source UUIDs also require adoption and remain blocked at the current pin.
  Token/QR join creates a fresh database and does not depend on adoption.

  When the API does land, adoption is more than accepting the mismatch: it MUST mint a new device
  UUID, generate a **new signing keypair** retiring the copied one (otherwise two replicas sign
  identically and writes cannot be attributed), and generate new Nostr and iroh endpoint keys.
  `initialize_in_place` runs before device-id reconciliation and seeds the HLC from the last
  persisted timestamp. The adopting revision must still test monotonic first writes with the new
  node ID and crash recovery across UUID, keys, index and restore markers.

  This is a dependency gap, not a scope decision. No text here restricts Holzi to a single machine.
