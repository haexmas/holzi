...
- **Device identity for HLC (`DeviceIdProvider`).** Decided 2026-09-07. holzi mints device UUIDs in
  ...
  Adopting a copied database on a second machine is an **intended way to add a replica**,
  ...
  explicit validation failure rather than permission to mint a replacement UUID.
  ...
  The HLC tolerates the new node identity because `initialize_in_place` runs before device-id
  reconciliation and seeds from the last persisted timestamp.
