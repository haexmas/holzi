...
**Onboarding-Härtung und Modellwahl-Persistenz — MVP-P1 in PR #27 (offen, 3 Commits `db4f47e` Slice A+B, `9f63f1f` Review-Fix runde 1, `b18c0b0` Review-Fix runde 2) am 2026-09-11**. Design in [ADR-0001](../docs/adr/0001-device-scoped-data-convention.md) festgelegt am 2026-09-10 nach Grill-Session; PR #26 mit den Design-Artefakten am 2026-09-11 08:31 UTC gemergt (Merge-Commit `2308e3e`). Der MVP-P1-Code-Slice deckt Phasen 1-4 aus [tasks.md](../specs/002-onboarding-model-prefs/tasks.md) ab (Foundational + US1 Onboarding-Wizard + US2 last-active Persistenz); Phase 5+6 (Settings-Screen mit `DefaultModelSetting` + `AliasSetting`), Phase 7 (US5 zusätzlicher Test-Case) und Phase 8 (Polish) sind Follow-up-PRs. Alle 9 CodeRabbit-Findings adressiert plus zusätzliche Härtungen in Runde 2: send_message schreibt die `chat.last_active_model_id`-Preference erst NACH erfolgreichem stream-Start (kein Rollback-Cross-Send-Race mehr), parallele Download/Import-Operationen auf denselben Slug werden per In-Process-Mutex serialisiert (`MODEL_PUBLICATION_LOCKS` in `models::paths`), und Frontend resettet `activeModel` bei loadModel-Failure. Der PR ändert:
...
- **Migration 0012 DROP `device_downloaded_models_no_sync`**: Filesystem unter `AppLocalData/models/<slug>/` wird alleinige Wahrheit für installiert-Sein. Downloads und Importe veröffentlichen finale reguläre `.gguf`-Dateien per atomarem Rename; `list_installed_models` scannt readdir, wählt bei mehreren Dateien den lexikografisch kleinsten Dateinamen deterministisch und joint mit `models`-Row. sha256/verified_at fallen weg (mistralrs' Load-Fehler ist die praktische Integritätsprüfung).
...
- **Workspace-Landing + FAB**: nach Vault-Open landet der Nutzer auf `/workspace/[instance]` (Instanzname + FAB unten rechts) statt direkt auf `/chat`. Der Chat wird vom FAB als Route (`/chat/[instance]`) aufgerufen; Middleware `onboarded` sichert Wizard-Erzwingung. Weitere Widgets folgen als eigenes Feature.
...
- `send_message` implementiert Preference-Rollback bei stream-Start-Failure, aber noch keinen expliziten `idempotencyKey` per Contract §send_message. User-Message + Preference commiten sequentiell, nicht atomar mit Retry-Semantik. Follow-up-Ticket.
