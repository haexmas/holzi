...
- **Preference-Writes** — nur bei intentionaler Nutzung: `chat.last_active_model_id` schreibt auf manuellen Picker-Wechsel ODER `send_message`. `chat.default_model_id` schreibt nur via expliziten "Als Default"-Button (Scope-Wahl: dieses Gerät ↔ vault-weit).
- **Onboarding-Route** `/onboarding/[instance]`: eigene Screen, Trigger `known_devices[my_installation_uuid].alias IS NULL`. Wizard-Schritte: (a) Gerätename (Placeholder = OS-Hostname wenn ermittelbar, sonst "Neues Gerät", Pflichtfeld — Onboarding ist fertig sobald `alias IS NOT NULL`), (b) Modellwahl mit drei Hardware-Fit-Vorschlägen "Easy" (kleinster mit `Fits`) / "Sweet" (größter mit `Fits`) / "Max" (größter mit `Fits` oder `Tight`) via neuem `catalog::recommend_tiers(&HardwareInfo)`, plus Hinweis "oder einen Anbieter hinterlegen". Kein Skip.
