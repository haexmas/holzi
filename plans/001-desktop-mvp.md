...
**Onboarding-Härtung und Modellwahl-Persistenz — MVP-P1 in PR #27 (offen, 3 Commits `db4f47e` Slice A+B, `9f63f1f` Review-Fix runde 1, `b18c0b0` Review-Fix runde 2) am 2026-09-11**. Design in [ADR-0001](../docs/adr/0001-device-scoped-data-convention.md) festgelegt am 2026-09-10 nach Grill-Session; PR #26 mit den Design-Artefakten am 2026-09-11 08:31 UTC gemergt (Merge-Commit `2308e3e`). Der MVP-P1-Code-Slice deckt die Phasen 1-3 vollständig und Phase 4 teilweise aus [tasks.md](../specs/002-onboarding-model-prefs/tasks.md) ab (Foundational + US1 Onboarding-Wizard + der implementierte Teil von US2 zur Last-active-Persistenz). Offen bleibt insbesondere T045: `idempotencyKey`, stabile Retry-IDs und die atomare Accepted-Send-Transaktion; das ist als Follow-up dokumentiert. Phase 5+6 (Settings-Screen mit `DefaultModelSetting` + `AliasSetting`) sind Follow-up-PRs. Phase 7 (US5) ist durch den FK-Cascade-Test aus Phase 2 bereits abgedeckt; offen ist dort nur die formale Verifikation in T060. Phase 8 (Polish) ist durch PR #27 und diesen Dokumentations-PR teilweise erledigt; verbleibende Verifikation und Aufräumarbeiten sind Follow-ups. Alle 9 CodeRabbit-Findings adressiert plus zusätzliche Härtungen in Runde 2: `send_message` schreibt die `chat.last_active_model_id`-Preference erst NACH erfolgreichem stream-Start; bei einem Startup-Fehler werden die bereits gestagte User-Message und der Thread zurückgerollt, nicht aber eine neue Preference persistiert. Dadurch gibt es kein Rollback-Cross-Send-Race mehr. Parallele Download/Import-Operationen auf denselben Slug werden per In-Process-Mutex serialisiert (`MODEL_PUBLICATION_LOCKS` in `models::paths`), und das Frontend resettet `activeModel` bei loadModel-Failure. Der PR ändert:
...
- `INSERT ... ON CONFLICT DO UPDATE` bricht in haex-crdt auf CRDT-getrackten Tabellen mit exakt einem tracked Column (`UNIQUE constraint failed: haex_crdt_dirty_tables_no_sync.table_name`). Workaround in `storage::preferences::insert_or_update`: check-then-write. Aufwärts als [haexmas/haex-crdt#28](https://github.com/haexmas/haex-crdt/issues/28) tracked (Fix-Vorschlag: Trigger-Body auf `ON CONFLICT` statt `OR REPLACE`).
- `send_message` schreibt die Preference erst nach erfolgreichem stream-Start; bei einem Startup-Fehler werden die gestagte User-Message und der Thread zurückgerollt, nicht eine zuvor geschriebene Preference. Es fehlt weiterhin der explizite `idempotencyKey` aus Contract §send_message; User-Message + Preference committen daher noch nicht atomar mit Retry-Semantik. Follow-up-Ticket.
- `list_installed_models` propagiert einen `canonical_model_file`-Ambiguity-Err (mehrere finalized GGUFs pro Slug) als Gesamt-Listen-Fehler; ein einzelner defekter Slug macht die komplette Liste unbrauchbar. `resolve_default_model` toleriert das schon per `if let Ok(Some(_))`. Follow-up: dieselbe Toleranz in `list_installed_models`.
