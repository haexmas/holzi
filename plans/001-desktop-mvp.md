...
- **Migration 0012 DROP `device_downloaded_models_no_sync`**: Filesystem unter `AppLocalData/models/<slug>/` wird alleinige Wahrheit für installiert-Sein. `list_installed_models` scannt readdir und joint mit `models`-Row. sha256/verified_at fallen weg (mistralrs' Load-Fehler ist die praktische Integritätsprüfung).
...
- **Onboarding-Route** `/onboarding/[instance]`: eigene Screen, Trigger `known_devices[my_installation_uuid].alias IS NULL`. Wizard-Schritte: (a) Gerätename (Input vorbefüllt mit OS-Hostname wenn ermittelbar, sonst mit "Neues Gerät", Pflichtfeld — Onboarding ist fertig sobald `alias IS NOT NULL`), (b) Modellwahl mit drei Hardware-Fit-Vorschlägen "Easy" (kleinster mit `Fits`) / "Sweet" (größter mit `Fits`) / "Max" (größter mit `Fits` oder `Tight`) via neuem `catalog::recommend_tiers(&HardwareInfo)`, plus Hinweis "oder einen Anbieter hinterlegen". Kein Skip.
