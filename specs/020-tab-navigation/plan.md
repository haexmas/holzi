...
| FR-012–014 (Öffnen an einem Ort, Legacy, unbekannter Ort)  | `shell.app.open`, `useShellTab().openApp`, `?at=`, Wurzel-`ShellRouterView`                                    |
...
| FR-018 (Maus)                                              | `ShellWindow` (`mouseup`/`auxclick`), Spike R6                                                                 |
| FR-019–020 (System-Zurück, Webview-Historie)               | nativer Hook → `shell.system.back`, Abwehr in `pages/workspace/[instance].vue`                                 |
...
| FR-028 (lesende Aktionen)                                  | `shell.state.get`, `shell.tab.history`, `shell.apps.list`, `shell.actions.list`, `chat.*.list`, `settings.get` |
...
1. **Reine Module** `navigation.ts`, `routeMatch.ts`, `keybindings.ts`,
   `lib/actions/{types,schema,runner,scopes}.ts` samt `check:shell-navigation`
   (Test zuerst).
...
5. **Titelleiste**: `ShellNavButtons`, `ShellHistoryMenu`; Spike Maustasten und
   Webview-Einstellungen (quickstart §3, R17), dann Maus-Anbindung.
6. **Tastatur, Abwehr, System-Zurück**: `useShellKeyboard`, Host-Seite, Hook.
