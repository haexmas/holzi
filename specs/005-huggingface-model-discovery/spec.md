...
- Q: Wie wird die lokale Modell-ID für frei installierte Hugging-Face-Modelle gebildet? → A: Deterministisch aus Hugging-Face-Repository-ID und Dateiname abgeleitet (Slug); nicht nutzer-editierbar. Der Anzeigename bleibt ein separates, frei editierbares Feld.
...
- Zusätzliche Klarstellung: Vor jedem Laden eines lokalen Modells muss Holzi den SHA-256-Hash der tatsächlich gefundenen Datei berechnen und gegen den gespeicherten Geräte-Hash prüfen. Bei einer Abweichung oder fehlender Integritätsbasis darf das Modell nicht geladen werden.
...
- Der Nutzer entfernt ein installiertes Modell nach Abschluss des Onboardings über den dauerhaften „Modelle verwalten"-Bereich: Nur die lokalen Modellbytes werden mit Bestätigung gelöscht; synchronisierte/nicht-lokale Metadaten, andere lokale Modelle sowie die Fallback-/Last-Active-Kette aus Spec 002 bleiben konsistent, auch wenn das entfernte Modell zuvor das aktive Modell war.
...
- **FR-009**: Das System MUSS die lokale Modell-ID für frei installierte Modelle deterministisch aus Repository-ID und Dateiname ableiten (Slug); die Modell-ID ist für den Nutzer nicht editierbar, ein separater Anzeigename bleibt frei änderbar. Modell-ID, Repository-ID, Dateiname und Tokenizer-Repository MÜSSEN so persistiert werden, dass ein späterer Load ohne erneute Suche möglich ist.
...
- **FR-026**: Bei fehlender Datei, fehlendem gespeicherten Hash, Hash-Abweichung oder Hash-/Dateisystemfehler MUSS der Ladevorgang mit einem strukturierten, lokalisierbaren Integritätsfehler abgebrochen werden. Das System DARF die erwartete Referenz nicht automatisch aktualisieren und keinen Runtime-Load starten.
...
- **SC-009**: In allen erfolgreichen lokalen Load-Testfällen wird die Datei vor dem Runtime-Start gehasht und gegen den gespeicherten Geräte-Hash geprüft; in allen Mismatch-/Missing-Hash-Fixtures startet kein Runtime-Load.
