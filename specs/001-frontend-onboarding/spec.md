...
**Dependency limitation, not a scope boundary**: adopting an unknown device UUID needs an adopt-on-mismatch API that the pinned haex-crdt revision does not expose — `reconcile_device_id` rejects a mismatch outright. Until it lands, a source from another machine is staged as pending and `open_instance` fails with an explicit `DeviceIdMismatch`; holzi MUST NOT mint a replacement UUID behind the operator's back. When it lands, adoption additionally mints a new device UUID, generates a new signing keypair retiring the copied one, and generates new Nostr and iroh endpoint keys — which is what rekey-on-restore already does for the same-machine case.

**Independent Test**: Place a valid holzi `.db` file whose UUID is present in the local device-ID index outside `<AppLocalData>/instances/`; open the Öffnen sheet, pick the file; verify (1) the file is copied (not moved) into `instances/`, (2) the source file remains untouched, (3) the copied file appears in the list, (4) unlocking with the original passphrase performs rekey-on-restore before any relay or iroh endpoint starts, (5) the federation view offers **Wiederherstellung verbinden**, (6) scanning or pasting a parent token updates that same imported database with mutually signed `peer_instances` records, and (7) no second instance file is created. Separately verify that a file with an unknown UUID is retained as pending and fails with an explicit `DeviceIdMismatch`; it is never opened with a silently minted UUID.

**Acceptance Scenarios**:

1. **Given** a valid `.db` file with a matching local UUID mapping, **When** the operator selects it in the Öffnen sheet, **Then** the file is copied into `instances/`, appears in the list, and its original passphrase unlocks the copy for rekey-on-restore; no network endpoint starts with the copied identity, and the federation view offers the restore-pairing handoff for the same database.
...
6. **Given** a `.db` file copied from another machine, whose device UUID is not in the local device-ID index, **When** the operator imports it and enters the correct passphrase, **Then** `open_instance` fails with an explicit `DeviceIdMismatch`, the copy and its pending marker are retained, and no UUID is minted silently. Once haex-crdt exposes adopt-on-mismatch, this case instead adopts a fresh device UUID, signing keypair, and endpoint keys, and continues into the same restore-pairing handoff as scenario 1.
...
- **FR-011**: Upon selection, the backend MUST validate the source as a regular `.db` file before copying. SQLCipher credential validation MUST occur in `open_instance`, using the passphrase entered in the Unlock sheet; a failed unlock attempt MUST return an explicit error without deleting the pending imported copy or marker. Rekey-on-restore MUST occur only for an unprocessed import-pending copy, before any Nostr relay or iroh endpoint starts: atomically replace the singleton `instance_identity_no_sync` row with fresh keys, retire the copied identity without using it on the network, and open the federation view in `restore-pairing-required` state. A restore-pending marker or persisted restore-pairing-required state is authoritative, including when both it and a stale import-pending marker exist after interruption; in that case `open_instance` MUST skip rekey, safely remove only the stale import marker, and preserve restore state until `pair_restored_instance` succeeds. Deletion requires an explicit discard action or conclusive validation that the file is not a holzi instance.
- **FR-011a**: A source whose device UUID is **not** in the local device-ID index requires adopting a new UUID. Against the pinned haex-crdt revision this is unavailable, so `open_instance` MUST fail with an explicit `DeviceIdMismatch`, retain the copy and its marker, and MUST NOT mint a replacement UUID. This is a dependency limitation; adopting a copied database is an intended replica-adding path, and no requirement here restricts holzi to a single machine. When the adopt API lands, this case MUST mint a fresh device UUID, generate a new signing keypair retiring the copied one, generate new Nostr and iroh endpoint keys, and then continue into the same restore-pairing handoff as FR-011.
- **FR-012**: The backend MUST copy (not move) the file into `<AppLocalData>/instances/` preserving its filename basename and mark the copy as pending validation until successful rekey and restore pairing. The local device-ID index is updated only after the source UUID mapping has been validated; an unknown mapping remains pending and is not opened.
