...
### User Story 3 — Öffnen (Import external `.db` file from filesystem) (Priority: P2, same-host scope)

The operator has an instance file (`.db`) on the same host filesystem and a matching entry in Holzi's local device-ID index, and wants holzi to manage it. Cross-host restore and arbitrary backups are deferred until haex-crdt provides adopt-on-mismatch support. On the landing they choose **Öffnen**, pick the file via OS file dialog, and holzi copies it into `<AppLocalData>/instances/` under its original name (or a de-duplicated variant). The copy is treated as a same-host restore: after the original passphrase unlocks it, holzi performs rekey-on-restore before starting any relay or iroh endpoint, atomically replaces the local identity row, retires the copied identity, and creates fresh instance keys. Holzi then opens the federation view for that same imported database in `restore-pairing-required` state. The operator selects **Wiederherstellung verbinden**, scans or pastes a pairing token from a surviving parent instance, and holzi completes pairing in place; it never creates a second database or starts on the copied keys.

**Why this priority**: Local portability. Holzi has no export command because the `.db` file *is* the export; Öffnen is the corresponding import. Cross-device migration requires a later haex-crdt adopt-on-mismatch revision and is not a v1 requirement.

**Independent Test**: Place a valid same-host holzi `.db` file whose UUID is present in the local device-ID index outside `<AppLocalData>/instances/`; open the Öffnen sheet, pick the file; verify (1) the file is copied (not moved) into `instances/`, (2) the source file remains untouched, (3) the copied file appears in the list, (4) unlocking with the original passphrase performs rekey-on-restore before any relay or iroh endpoint starts, (5) the federation view offers **Wiederherstellung verbinden**, (6) scanning or pasting a parent token updates that same imported database with mutually signed `peer_instances` records, and (7) no second instance file is created. A file without a local UUID mapping is retained as pending and fails explicitly; it is not opened with a newly minted UUID.

**Acceptance Scenarios**:

1. **Given** a valid same-host `.db` file with a matching local UUID mapping, **When** the operator selects it in the Öffnen sheet, **Then** the file is copied into `instances/`, appears in the list, and its original passphrase unlocks the copy for rekey-on-restore; no network endpoint starts with the copied identity, and the federation view offers the restore-pairing handoff for the same database.
...
**Independent Test**: Not applicable; see the same-host User Story 3 restore-pairing test. Cross-host restore is not a v1 requirement.
...
- **FR-011**: Upon selection, the backend MUST validate the source as a regular `.db` file before copying and require a matching UUID in the local device-ID index before opening it. SQLCipher credential validation MUST occur in `open_instance`, using the passphrase entered in the Unlock sheet; a failed unlock attempt, including an incorrect passphrase or missing UUID mapping, MUST return an explicit error without deleting the pending imported copy or marker. Rekey-on-restore MUST occur only for an unprocessed same-host import-pending copy, before any Nostr relay or iroh endpoint starts: atomically replace the singleton `instance_identity_no_sync` row with fresh keys, retire the copied identity without using it on the network, and open the federation view in `restore-pairing-required` state. A restore-pending marker or persisted restore-pairing-required state is authoritative, including when both it and a stale import-pending marker exist after interruption; in that case `open_instance` MUST skip rekey, safely remove only the stale import marker, and preserve restore state until `pair_restored_instance` succeeds. Cross-host restore is rejected as unsupported in v1. Deletion requires an explicit discard action or conclusive validation that the file is not a holzi instance.
