...
- Pairing offers QR scanning through the joiner's camera as the primary path, with text-token input as fallback. Both paths carry exactly the same encoded token; the QR is only a transport for that string. The QR-rendering surface on the parent device (which produces the token the joiner scans) lives outside this spec's landing scope and is covered by the federation-view spec.
