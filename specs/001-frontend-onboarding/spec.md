...
The operator has an instance file (`.db`) on the filesystem — moved from another machine, restored from backup, or simply outside the managed directory — and wants holzi to manage it. On the landing they choose **Öffnen**, pick the file via OS file dialog, and holzi copies it into `<AppLocalData>/instances/` under its original name (or a de-duplicated variant). Once the adoption dependency below is satisfied, the copy is adopted as a new replica: after the original passphrase unlocks it, and before any relay or iroh endpoint starts, holzi issues a **handover attestation** with the still-available copied signing key, adopts a fresh device UUID, atomically replaces the local identity row, retires the copied identity, and creates fresh instance keys. It never creates a second database or starts on the copied keys.

Because the attestation authorizes the adopted identity against a key every existing peer already trusts, adoption **completes offline** and opens the normal federation view — no token, no reachable parent (operator decision, 2026-09-08). `restore-pairing-required` remains only as the fallback for a copy from which no valid attestation can be produced, for example a database whose signing key is missing or unusable; that state then offers **Wiederherstellung verbinden** as before.
...
1. **Given** a valid `.db` file and a reviewed adoption-capable dependency, **When** the operator imports and unlocks it, **Then** a handover attestation is written with the copied signing key before that key is retired, the copy receives a fresh UUID and signing/endpoint keys before application writes or network startup, the source UUID, keys and mapping remain unchanged, and the normal federation view opens **without requiring a token or a reachable parent**. At the current pin, scenario 6 applies instead.
1a. **Given** an adopted replica that has never been online, **When** it later reaches any peer that trusts the source instance's public key, **Then** that peer verifies the handover attestation and accepts the replica on first contact, and the replica appears in that peer's UI as a newly derived device rather than being added silently.
...
- **FR-011**: Upon selection, the backend MUST validate the source as a regular `.db` file before copying. SQLCipher credential validation MUST occur in `open_instance`, using the passphrase entered in the Unlock sheet; a failed unlock attempt MUST return an explicit error without deleting the pending imported copy or marker. Subject to the adoption gate in FR-011a, rekey-on-restore MUST occur only for an unprocessed import-pending copy, before any Nostr relay or iroh endpoint starts: issue the handover attestation defined in FR-011b while the copied signing key is still usable, then atomically replace the singleton `instance_identity_no_sync` row with fresh keys and retire the copied identity without using it on the network. On success the normal federation view opens. `restore-pairing-required` is entered **only** when no valid attestation could be produced. Restore completion is authoritative only when its operation ID and adopted UUID match this local import generation, as defined in the command contract; a restore flag copied from the source MUST NOT bypass adoption. With that proof, restore state takes precedence over a stale import-pending marker after interruption; in that case `open_instance` MUST finish any interrupted target-index/marker publication, skip adoption and rekey, safely remove only the stale import marker, and preserve restore state until `pair_restored_instance` succeeds. Deletion requires an explicit discard action or conclusive validation that the file is not a holzi instance.
...
- **FR-011b**: Adoption MUST complete offline, without a reachable parent (operator decision, 2026-09-08). Authorization comes from a **handover attestation**: while the copied signing key is still available and **before** it is retired, the backend MUST sign a record stating that the source instance's public key attests the adopted instance's new public key as a derived replica, with the adoption timestamp, and MUST write it into `peer_instances` as the adopted instance's own entry. Only then may the copied key be retired and the endpoint keys rotated. Any peer MUST be able to verify that attestation against a public key already present in its own `peer_instances` and accept the replica on first contact. Because this turns one-time file access into ongoing membership that a passphrase change does not revoke, a peer MUST be able to revoke a derived replica by bumping its revocation epoch, and the UI MUST surface newly appeared derived replicas rather than adding them silently. If no valid attestation can be produced — for example a missing or unusable signing key — the import MUST fall back to `restore-pairing-required` and the token flow in FR-014a.
- **FR-012**: The backend MUST copy (not move) the file into `<AppLocalData>/instances/` preserving its filename basename and mark the copy as pending validation until successful adoption, rekey and restore pairing. Staging MUST NOT register the source UUID as the target identity. After adoption becomes available, the target index entry records the fresh adopted UUID as part of the recoverable restore transition; the source mapping remains unchanged. Until then the copy remains import-pending and cannot be opened.
