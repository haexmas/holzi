...
1. **Given** a valid `.db` file with a matching local source UUID mapping and a reviewed adoption-capable dependency, **When** the operator imports and unlocks it, **Then** the copy receives a fresh UUID and signing/endpoint keys before application writes or network startup, while the source UUID, keys and mapping remain unchanged. The federation view offers the restore-pairing handoff for that same copy. At the current pin, scenario 6 applies instead.
