...
The operator has an already-attested device (parent) running holzi and installs holzi on a second device (joiner). On the joiner's landing they choose **Verbinden**, scan a short-lived QR code (or paste a token) that the parent displays, set a local passphrase, and complete pairing. The joiner's new instance is created, the pairing transcript is co-signed, and federation state syncs from the parent via `haex-crdt`.
...
3. **Given** a token with a mismatched federation epoch (e.g., stale QR), **When** the joiner submits it, **Then** pairing fails with a clear "federation state changed" error.
...
- **App closed mid-Anlegen (before paper-seed confirmation)**: on next launch, an orphan file may exist. Behavior: on startup, delete any file in `instances/` whose creation flag `.pending` still exists in the same directory. The pending flag is written before the DB and removed only after paper-seed confirmation. Details are spec-phase for the Tauri backend.
- **Two instances open concurrently**: prevented by backend rejecting `open_instance` while one is active. Frontend must call `close_instance` before opening another. Race condition surfaces as an error, not a corrupt state.
...
- **FR-007**: On successful submit, the backend MUST create `<AppLocalData>/instances/<name>.db`, initialize `haex-crdt` with the passphrase, run the mode-specific initialization, and return the paper-seed (Genesis) or matched fingerprint (Recover).
- **FR-008**: The paper-seed MUST be displayed for reading in a Genesis flow; the operator MUST explicitly confirm they recorded it before the Anlegen sheet closes.
- **FR-009**: If Anlegen is cancelled after the DB file is created but before paper-seed confirmation, the backend MUST discard the file so no orphan state remains.
...
- **FR-011**: Upon selection, the backend MUST validate the file is a readable SQLCipher database and reject with an explicit error if not, before copying.
- **FR-012**: The backend MUST copy (not move) the file into `<AppLocalData>/instances/` preserving its filename basename.
...
- **FR-015**: The Verbinden action MUST open a Sheet with a QR scanner (mobile + desktop webcam if available) and a fallback text-input field for the pairing token.
- **FR-016**: The Verbinden sheet MUST require: instance name (as FR-006), passphrase (as FR-006), pairing token (from QR or text).
...
- **FR-022**: If an instance is already active, opening a new instance MUST first close the active one atomically (backend contract).
- **FR-023**: The list MUST support a per-item context menu with "Aus Liste entfernen (Datei behalten)" and "In Papierkorb verschieben" actions. Silent hard-delete MUST NOT be an option.

**Cross-cutting**

- **FR-024**: All backend mutations to `instances/` (create, open, close, import, trash) MUST emit `instance-list-changed` events; the Pinia store MUST subscribe and re-sync.
- **FR-025**: The frontend MUST NEVER pass file paths to backend commands; all commands MUST use instance names (basename without `.db`). Paths are the backend's authority.
...
- **InstanceInfo**: metadata surface for the frontend list: `{ name: string, lastAccess: ISO8601, sizeBytes: number }`. No secrets.
...
- **SC-002**: With a valid pairing token, Verbinden completes in under 30 seconds on the same local network, from QR scan to federation view.
...
- `<AppLocalData>` on each platform is the app-private data directory Tauri resolves via `BaseDirectory::AppLocalData` (Linux: `~/.local/share/holzi/`, macOS: `~/Library/Application Support/holzi/`, Windows: `%APPDATA%\holzi\`, Android: app-private storage, iOS: app sandbox `Library/Application Support/`).
...
- The QR scanner uses the host webcam on desktop where available, and native camera APIs on Android/iOS via a Tauri plugin (spike-selected). Fallback text input is always present.
