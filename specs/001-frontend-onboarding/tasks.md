...
- [ ] **T051** [P] [US3] Rust test: `import_instance_file` with a valid throwaway encrypted instance copies atomically to a pending target, leaves source untouched, emits `instance-list-changed { reason: 'imported' }`, and on unlock with the supplied passphrase rekeys before any relay/iroh startup, retires the copied identity, and clears the marker only after the fresh identity enters pairing as a new peer.
