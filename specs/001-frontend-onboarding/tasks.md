...
`DeviceIdProvider` inserting a `known_devices` row keyed by the local
installation UUID. Do not implement `PaperSeedDisplay`, `paper_seed`,
...
**Independent Test**: Import a valid throwaway database; verify source untouched, list refreshed, and that opening it inserts a fresh `known_devices` row for the local installation UUID with a vault-device UUID distinct from any source row. Reopening reuses the same row; no new one is inserted.

### Tests for US3

- [ ] **T051** [P] [US3] Rust tests: (1) `import_instance_file` with a valid throwaway encrypted instance writes and fsyncs the destination file, leaves the source untouched, emits `instance-list-changed { reason: 'imported' }` only after the copy is durable, and cleans up on a mid-copy crash without leaving a partial database that startup would treat as importable; (2) opening an imported copy with the correct passphrase inserts a `known_devices` row keyed by the local installation UUID with a fresh vault-device UUID, and starts relay/iroh under that identity; (3) reopening the same copy on the same install reuses that row; (4) opening the same copy on a different install (simulated by pointing at a different installation-id file) inserts a second row and returns a different vault-device UUID; (5) an overwrite crash before staged-database rename cannot leave a partial file that opens; startup detects and removes it.
...
- [ ] **T055** [US3] Implement `src-tauri/src/instances/import.rs::import_instance_file` per contract, including regular-file/extension validation, crash-safe staging (`.db.importing` → atomic rename), conflict handling, and active-target rejection under the state lock. SQLCipher credential validation is deferred to `open_instance`. No rekey, no attestation, no restore marker — the fresh `known_devices` row is what makes the copy a distinct replica, and that row is inserted by the `DeviceIdProvider` during `open_instance`.
