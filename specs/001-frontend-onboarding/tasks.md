...
**Independent Test**: Import a valid throwaway database from the same installation and verify source untouched, list refreshed, and local `known_devices` row reuse. Repeat from a second installation and verify a fresh local row with a vault-device UUID distinct from the source. Reopening under either installation reuses the same row; no new one is inserted.

### Tests for US3

- [ ] **T051** [P] [US3] Rust tests: (1) `import_instance_file` with a valid throwaway encrypted instance writes and fsyncs the destination file, leaves the source untouched, emits `instance-list-changed { reason: 'imported' }` only after the copy is durable, and cleans up on a mid-copy crash without leaving a partial database that startup would treat as importable; (2) opening an imported copy with the correct passphrase reuses the local-only `known_devices` row when it came from the same install, otherwise inserts a row keyed by the local installation UUID with a fresh vault-device UUID, and starts relay/iroh under that identity; (3) reopening the same copy on the same install reuses that row; (4) opening the same copy on a different install (simulated by pointing at a different installation-id file) inserts a second row and returns a different vault-device UUID; (5) an overwrite crash before staged-database rename cannot leave a partial file that opens; startup detects and removes it.
