...
a fresh per-instance identity and use offline attested adoption for imported
databases, with restore pairing only as the token fallback. Do not implement `PaperSeedDisplay`, `paper_seed`,
...
- [ ] **T025** [US4] Implement `src-tauri/src/instances/crud.rs::open_instance` per contract. Under the state lock, validate the requested instance and SQLCipher credentials before changing any current active runtime; on validation failure, preserve the existing runtime and `AppState.active_instance` unchanged. For an import-pending copy, enforce FR-011a at the current pin; after a reviewed adoption-capable upgrade, perform the attestation/adoption transaction from the contract, publish the fresh UUID mapping and remove the import marker before starting relay/iroh under the fresh identity. Only the token fallback publishes a restore marker; attested completion returns `restore_pairing_required = false` offline. On an adopted copy whose completion record matches the local import operation and adopted UUID, finish interrupted index/marker publication, skip adoption/rekey and preserve the committed attested or fallback outcome after restart or startup failure. Start the requested SQLCipher/relay/iroh runtime as a private candidate while the old runtime remains active; close the old runtime and publish the candidate only after every startup step succeeds. On candidate startup failure, stop only candidate services, drop its handle, and retain the old runtime and `AppState.active_instance`, along with any restore database and marker. Permit deletion only after an explicit discard action or conclusive validation that the file is not a holzi instance. Enforce `NotFound` and `WrongPassphrase` return the same discriminator-different-but-message-same guarantee. Add tests for active-instance preservation on validation failure and forced partial startup failure, plus restore retry after interruption.
...
**Goal**: Anlegen CTA opens the Create sheet; Genesis mode creates a fresh instance with fresh per-instance identities. Backup recovery is handled by Öffnen with offline attested adoption or the token fallback.

**Independent Test**: On an empty install, click Anlegen, submit valid inputs, and land in the federation view. New file exists in `instances/` with fresh `instance_identity_no_sync` keys.
...
- [ ] **T033** [P] [US1] `e2e/onboarding.spec.ts::"anlegen interruption discards orphan"`: interrupt Genesis after the database is created but before activation completes, then relaunch and verify startup cleanup removes the Genesis `.pending` marker and sibling file from `list_instances`.
...
- [ ] **T041** [P] [US1] i18n: `onboarding.create.title`, `onboarding.create.name`, `onboarding.create.passphrase`, `onboarding.create.passphraseConfirm`, `onboarding.create.submit`, `errors.nameConflict`, `errors.invalidName`, `errors.weakPassphrase`.
...
- [ ] **T042** [US2] `e2e/two-device-pairing.spec.ts`: launch two isolated Tauri app instances, have the parent issue a token, paste it into the joiner, verify relay delivery and `haex-crdt` synchronization of mutually signed `peer_instances` records, then send the walking-skeleton ping and assert the pong. This is the MVP acceptance test.
...
**Independent Test**: Import copies with both known and unknown source UUIDs; verify source untouched and list refreshed. At the current pin both remain import-pending and fail with the explicit adoption-unavailable `CrdtInit` error, leaving index and active instance unchanged and starting no endpoint. Ordinary reopen and fresh-database Token-Join remain independent of this gate.

### Tests for US3

Restore-success checks below require a reviewed adoption-capable crate pin. At the current pin, test the FR-011a rejection for both known and unknown source UUIDs; keep the future success checks tracked as blocked, not passed.

- [ ] **T051** [P] [US3] Rust tests: (1) `import_instance_file` with a valid throwaway encrypted instance writes and fsyncs the import marker before publishing the database, leaves the source untouched, emits `instance-list-changed { reason: 'imported' }` only after both are durable, and injects a crash at the rename boundary without allowing an unmarked imported database to open normally; (2) opening an import-pending copy writes a handover attestation signed by the copied key before retiring it, adopts a UUID distinct from the source, rotates signing/endpoint keys before any application write or relay/iroh startup, and reaches the normal federation view with no network available; a peer whose effective trust store authorizes the source grant verifies that attestation and accepts the replica; inject interruption across attestation, adoption, rekey, index and marker publication and verify safe recovery, in particular that the attestation and its exact fresh private keys survive together or the transaction rolls back to the copied identity; for the explicit token fallback verify durable fallback completion instead, and ensure a storage failure rolls back rather than silently choosing fallback; (3) after a crash or forced candidate startup failure, reopening an attested copy preserves the same UUID, keys and attestation, finishes index/marker publication and returns `restore_pairing_required = false` without a token; separately verify that the token fallback preserves `restore_pairing_required = true` and its restore marker for pairing retry; a crash after fallback pairing commits but before marker deletion must clean up without pairing or rekeying again; (4) an overwrite crash before staged-database rename cannot apply a generation-specific marker to the old target and recovers through the overwrite journal; (5) importing a source that already contains restore-pairing-required state still receives a new local operation ID and cannot bypass the adoption gate or fresh UUID/key rotation; only matching local completion evidence permits retry without rotation.
- [ ] **T051a** [P] [US3] Attestation authorization tests after the reviewed dependency/protocol upgrade: verify canonical encoding/domain/version against fixed positive and negative vectors; reject changes to each signed identity/authorization field, substituted Nostr/iroh keys or absent endpoint-key proof before ordinary sync/application access; reject sources without current pairing authority, capability escalation, extended expiry, unknown/cyclic trust assertions, stale epochs and revoked source/child grants. Replay must not restore a revoked peer; identical valid attestations are idempotent. Recheck revocation on subsequent ingress. Test local offline success followed by rejection against newer remote trust and token reauthorization of the same UUID/keys, including restart during fallback-state publication and `instance-list-changed` emission only after the new state is durable; unauthenticated rejections or unreachability alone must preserve attested completion. Verify only authorized peers can revoke and every peer's UI visibly reports a newly accepted derived replica.
...
- [ ] **T055** [US3] Implement `src-tauri/src/instances/import.rs::import_instance_file` per contract, including regular-file/extension validation, source/target device-ID separation, crash-safe marker-before-publish staging, generation-bound overwrite journaling, conflict handling, and active-target rejection under the state lock. Defer SQLCipher credential validation and rekey-on-restore to `open_instance`; the copied keys MUST NOT reach relay/iroh startup. On success the adopted identity carries a handover attestation and enters the normal federation view offline (FR-011b); `restore-pairing-required` is the fallback when no valid attestation is available, including later invalidation under FR-011b. At the current pin all unprocessed copies remain import-pending and are rejected by `open_instance` with the explicit adoption-unavailable `CrdtInit` error; see FR-011a. After a reviewed dependency upgrade, coordinate fresh UUID adoption with key rotation and crash-safe index/marker publication; a known source UUID does not bypass adoption.
- [ ] **T055a** [US3] Implement `pair_restored_instance` per contract: accept the token from the federation-view restore handoff, update the active imported database in place with mutually signed `peer_instances` records, clear the pairing flag only in a successful peer-record transaction and then remove the restore marker durably; retain the completion record and same database/identity for retry and stale-marker recovery. Support the same fallback after a previously valid attestation is rejected against newer trust state, without another rekey.
