...
- [x] T045 [US2] Extend `send_message` in `src-tauri/src/chat/commands.rs`: require a stable non-empty `idempotencyKey`, persist it uniquely with the user message, and derive the user/assistant IDs deterministically from the key (UUID v5) so a retry of the frontend's own `invoke()` call returns the same `SendMessageResult` without a second insert or a second `stream_chat`; a retry with a different thread or content for the same key is `HolziError::InvalidInput`. Deliberately does NOT resume/restart a generation that failed after acceptance — that remains "send a new message with a new key" (simplified from an earlier contract draft, see contracts/tauri-commands.md §send_message). `preferences[('<my_device>', 'chat.last_active_model_id')] = session.model_id` still writes only after a fresh (non-duplicate) `stream_chat` start succeeds, unchanged from before this task.
