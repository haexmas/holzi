...
| Do not depend on wall-clock timing in tests (Rust testing rule) | Excepted | The elapsed time is what these scenarios check. Waits are polls with a deadline, never fixed sleeps, and `E2E_TIME_SCALE` scales the deadlines. Recorded in Complexity Tracking.                 |
...
| Timing assertions flake on a loaded machine                                     | The suite is switched off                   | Polls with deadlines, generous relation of the checks to the 4 s, 1 s and 10 s promises, `E2E_TIME_SCALE`, and the timeline in the report showing the slowdown before it becomes a failure (FR-020). |
...
| Wall-clock assertions (4 s, 1 s, 10 s)                                                    | The elapsed time is the promise under test.                                                                                                                 | A test that does not measure time cannot show "ends within seconds". Polls with deadlines and a scale factor keep it robust.                                                                                                               |
