...
are not needed.
Decisions, alternatives and the few unverified items are in [research.md](research.md).
...
dev-dependency (already on this branch). haex-crdt gets a revision bump, pinned by full commit SHA,
once its `SqlCipherKey` change lands.
...
| External dependency: haex-crdt `SqlCipherKey` zeroize                                                                                                                                                 | Only that crate can erase the key copy it holds                                                            | A wrapper in holzi cannot reach the `String` inside the crate; until it lands the residual is one freed string per open, bounded by the process                   |
