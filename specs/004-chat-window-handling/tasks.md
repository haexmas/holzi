...
- [X] T007 Add `modelLoadStatusAsync`, `onModelLoadProgress`, and `onModelLoadError` wrappers to `src/composables/useChat.ts`; ignore events whose `vaultGeneration` does not match the active Vault, and stale events by `loadId` within the same generation.
