...
| `tool_source` | TEXT | YES | Nur bei `role = tool_call`. Einer von `built_in` / `mcp` / `cli`. (`acp_delegate` aus der Design-Doku ist für dieses Feature nicht erreichbar — `cli_delegate` ist out of scope, siehe spec.md.) |

**Validierungsregeln** (Rust-Wrapper, `storage/chat_messages.rs`):

- Eine `tool_call`-Zeile hat immer `tool_name`, `tool_call_id`, `tool_input` gesetzt; `content` bleibt
  leer (kein Doppel-Feld für dieselbe Information).
- Eine `tool_result`-Zeile hat immer `tool_call_id` gesetzt (auf denselben Wert wie ihre `tool_call`-
  Zeile) und trägt das Ergebnis in `content` (JSON- oder Klartext, je nach Tool).
...
| `execute(input)` | `async fn(Value) -> ToolResult` | `ToolResult { content: String, is_error: bool }`. |

**Edge Case Namenskollision** (aus spec.md Edge Cases nicht explizit behandelt, hier ergänzt): zwei
Quellen (z.B. ein Built-in-Tool und ein MCP-Server) liefern denselben `name()`. Registry-Aufbau MUST
das später hinzugefügte (MCP, da dynamisch nach Built-ins geladen) mit einem Quellen-Präfix
disambiguieren, statt eines der beiden stillschweigend zu verdecken — sonst wird FR-007
("jede Tool-Nutzung sichtbar") für den verdeckten Fall irreführend.

### `ToolRegistry` (`chat/tools/mod.rs`, in `ChatState`)

Hält `Vec<Box<dyn Tool>>` aus drei Quellen (Built-in beim App-Start, MCP dynamisch bei
Server-Verbindung, das eine Host-CLI-Tool statisch). Baut pro Step die `tools: Vec<ToolSpec>` für
`ChatRequest`.
...
Wire-Format-Mapping pro Adapter ist in research.md §1 (Anthropic) und §2 (mistralrs) fixiert.
