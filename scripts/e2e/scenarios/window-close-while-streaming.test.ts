...
// Spec 013: closing the window while a reply streams ends the process and cancels the reply within the
// same fixed deadlines as pressing lock (lock-while-streaming.test.ts) - the vault closes either way.
scenario('window-close-while-streaming', {}, async (ctx) => {
  ...
  assert.equal(
    alertBefore,
    false,
    'an alert was already showing just before the window close',
  )

  const closedWindowAt = Date.now()
  ctx.step('window-close')
  ...
  // A process that ends within milliseconds of the window close leaves the socket's own "close" event
  // no real time to reach this process's event loop yet; wait for it rather than reading closedAt once,
  // right after (research: the promise is 1 second from the window close, not from whenever this line
  // happens to run).
  await ctx.waitFor(
  ...
  const closeDelayMs = closedAt - closedWindowAt
  assert.ok(
    closeDelayMs <= PROVIDER_CLOSE_LIMIT_MS,
    `provider connection closed ${closeDelayMs} ms after the window close, over the ${PROVIDER_CLOSE_LIMIT_MS} ms limit`,
  )
