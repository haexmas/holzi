//! Tests for the Anthropic adapter's model-listing transport using
//! `wiremock` as a local HTTP double. Each test spins its own server on a
//! random port so tests can run in parallel without cross-talk.
//!
//! The streaming path lives in `anthropic_stream_tests.rs` and
//! request-body construction in `request_tests.rs`.
