//! Tauri command surface for the Shell layout (spec 015-workspace-shell,
...
//! keeps the actual logic callable from `shell_commands_tests.rs` without
...
use crate::storage::shell_windows::{self, ShellWindowError, TabWrite, WindowRow, WindowWrite};
use crate::storage::shell_workspaces::{self, WorkspaceDeleteError, WorkspaceRow};
...
impl From<shell_windows::TabRow> for TabDto {
    fn from(row: shell_windows::TabRow) -> Self {
        TabDto {
            tab_id: row.tab_id,
            app_id: row.app_id,
        }
...
fn shell_window_error_to_holzi(err: ShellWindowError) -> HolziError {
    let reason = err.to_string();
    match err {
        ShellWindowError::Sql(e) => HolziError::from(haex_crdt::Error::from(e)),
...
async fn load_layout(db: VaultDb, device: Uuid) -> Result<ShellLayoutDto> {
    tauri::async_runtime::spawn_blocking(move || -> Result<ShellLayoutDto> {
        let workspace_rows = db
            .with_connection(|conn| Ok(shell_workspaces::ensure_default(conn, device)?))
            .map_err(HolziError::from)?;
        let window_rows = db
            .with_connection(|conn| Ok(shell_windows::load_all(conn, device)?))
            .map_err(HolziError::from)?;
    ...
    .map_err(|e| HolziError::CrdtInit {
        reason: format!("shell_load_layout join: {e}"),
    })?
}

async fn create_workspace(db: VaultDb, device: Uuid) -> Result<WorkspaceDto> {
    tauri::async_runtime::spawn_blocking(move || {
        db.with_connection(|conn| Ok(shell_workspaces::create(conn, device)?))
    })
    .await
    .map_err(|e| HolziError::CrdtInit {
        reason: format!("shell_create_workspace join: {e}"),
    })?
    .map_err(HolziError::from)
    .map(Into::into)
}
...
/// deleted workspace was first (contract `shell_delete_workspace`). Mirrors
...
async fn delete_workspace(
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<DeleteWorkspaceResult> {
        ...
        let before = db
            .with_connection(|conn| Ok(shell_workspaces::list(conn, device)?))
            .map_err(HolziError::from)?;
        let deleted_index = before.iter().position(|w| w.workspace_id == workspace_id);

        let remaining = db
            .with_connection(|conn| Ok(shell_workspaces::delete(conn, device, workspace_id)))
            .map_err(HolziError::from)?
    ...
    .await
    .map_err(|e| HolziError::CrdtInit {
        reason: format!("shell_delete_workspace join: {e}"),
    })?
}
...
async fn set_active_workspace(db: VaultDb, device: Uuid, workspace_id: Uuid) -> Result<()> {
    ...
    .await
    .map_err(|e| HolziError::CrdtInit {
        reason: format!("shell_set_active_workspace join: {e}"),
    })?
}

async fn save_windows(db: VaultDb, device: Uuid, windows: Vec<WindowWrite>) -> Result<()> {
    tauri::async_runtime::spawn_blocking(move || -> Result<()> {
        db.with_connection(|conn| Ok(shell_windows::save_batch(conn, device, &windows)))
            .map_err(HolziError::from)?
            .map_err(shell_window_error_to_holzi)
    })
    .await
    .map_err(|e| HolziError::CrdtInit {
        reason: format!("shell_save_windows join: {e}"),
    })?
}

async fn close_windows_batch(db: VaultDb, device: Uuid, window_ids: Vec<Uuid>) -> Result<()> {
    tauri::async_runtime::spawn_blocking(move || -> Result<()> {
        db.with_connection(|conn| Ok(shell_windows::close_windows(conn, device, &window_ids)?))
            .map_err(HolziError::from)
    })
    .await
    .map_err(|e| HolziError::CrdtInit {
        reason: format!("shell_close_windows join: {e}"),
    })?
}
...
#[tauri::command]
pub async fn shell_load_layout(
    app: AppHandle,
    state: State<'_, AppState>,
) -> Result<ShellLayoutDto> {
...
#[tauri::command]
pub async fn shell_create_workspace(
    app: AppHandle,
    state: State<'_, AppState>,
) -> Result<WorkspaceDto> {
    let db = active_database(&state)?;
...
#[tauri::command]
pub async fn shell_delete_workspace(
    app: AppHandle,
    state: State<'_, AppState>,
...
#[tauri::command]
pub async fn shell_set_active_workspace(
    app: AppHandle,
    state: State<'_, AppState>,
    args: WorkspaceIdArgs,
) -> Result<()> {
...
#[tauri::command]
pub async fn shell_save_windows(
    app: AppHandle,
    state: State<'_, AppState>,
...
#[tauri::command]
pub async fn shell_close_windows(
    app: AppHandle,
    state: State<'_, AppState>,
