//! Tests for `shell_workspaces` (spec 015-workspace-shell). Every function
...
fn ensure_default_creates_exactly_one_workspace_on_an_empty_device() {
    let (_dir, db, device) = open_test_vault("shell-workspaces-ensure-default");
...
fn create_appends_at_a_dense_position() {
    let (_dir, db, device) = open_test_vault("shell-workspaces-create");
...
fn deleting_the_last_workspace_is_refused() {
    let (_dir, db, device) = open_test_vault("shell-workspaces-last");
    let only = db
        .with_connection(|conn| Ok(create(conn, device)?))
        .expect("create");
...
fn deleting_an_unknown_workspace_is_refused() {
    let (_dir, db, device) = open_test_vault("shell-workspaces-unknown");
...
fn deleting_a_middle_workspace_densely_renumbers_the_rest() {
    let (_dir, db, device) = open_test_vault("shell-workspaces-renumber");
    let first = db
        .with_connection(|conn| Ok(create(conn, device)?))
        .expect("create 0");
    let middle = db
...
fn deleting_a_workspace_with_windows_still_succeeds_via_the_fk_cascade() {
    ...
    let (_dir, db, device) = open_test_vault("shell-workspaces-cascade");
    let doomed = db
        .with_connection(|conn| Ok(create(conn, device)?))
