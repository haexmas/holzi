...
//! `wm_workspaces_tests` — these open a real `Database` directly.
...
use super::wm_windows::{
    close_windows, load_all, save_batch, TabWrite, WindowWrite, WmWindowError,
};
use super::wm_workspaces::create as create_workspace;
use crate::identity::{
...
fn save_batch_inserts_a_window_with_its_tabs_in_order() {
    let (_dir, db, device) = open_test_vault("wm-windows-insert");
    let workspace = db
...
fn save_batch_updates_geometry_and_replaces_tabs() {
    let (_dir, db, device) = open_test_vault("wm-windows-update");
    let workspace = db
        .with_connection(|conn| Ok(create_workspace(conn, device)?))
        .expect("create workspace");
...
fn save_batch_rejects_a_window_with_no_tabs() {
    let (_dir, db, device) = open_test_vault("wm-windows-empty-tabs");
    let workspace = db
        .with_connection(|conn| Ok(create_workspace(conn, device)?))
        .expect("create workspace");
    let window = WindowWrite {
    ...
    let outcome: std::result::Result<(), WmWindowError> = db
        .with_connection(|conn| Ok(save_batch(conn, device, &[window])))
        .expect("with_connection succeeds");
    assert!(matches!(outcome, Err(WmWindowError::TabCountOutOfRange)));
}

#[test]
fn save_batch_rejects_an_active_tab_id_outside_the_window() {
    let (_dir, db, device) = open_test_vault("wm-windows-bad-active-tab");
    let workspace = db
        .with_connection(|conn| Ok(create_workspace(conn, device)?))
        .expect("create workspace");
    let window = WindowWrite {
    ...
    let outcome: std::result::Result<(), WmWindowError> = db
        .with_connection(|conn| Ok(save_batch(conn, device, &[window])))
        .expect("with_connection succeeds");
    assert!(matches!(outcome, Err(WmWindowError::ActiveTabNotInWindow)));
}

#[test]
fn save_batch_rejects_a_tab_id_reused_across_two_windows() {
    let (_dir, db, device) = open_test_vault("wm-windows-duplicate-tab");
    let workspace = db
        .with_connection(|conn| Ok(create_workspace(conn, device)?))
        .expect("create workspace");
    ...
    let outcome: std::result::Result<(), WmWindowError> = db
        ...
        .expect("with_connection succeeds");
    assert!(matches!(outcome, Err(WmWindowError::DuplicateTabId)));
}

#[test]
fn save_batch_rejects_an_invalid_app_id() {
    let (_dir, db, device) = open_test_vault("wm-windows-bad-app-id");
    let workspace = db
        .with_connection(|conn| Ok(create_workspace(conn, device)?))
        .expect("create workspace");
    let tab_id = Uuid::new_v4();
    ...
    let outcome: std::result::Result<(), WmWindowError> = db
        .with_connection(|conn| Ok(save_batch(conn, device, &[window])))
        .expect("with_connection succeeds");
    assert!(matches!(outcome, Err(WmWindowError::InvalidAppId)));
}

#[test]
fn save_batch_rejects_geometry_outside_the_allowed_bounds() {
    let (_dir, db, device) = open_test_vault("wm-windows-bad-geometry");
    let workspace = db
        .with_connection(|conn| Ok(create_workspace(conn, device)?))
        .expect("create workspace");
    let window = WindowWrite {
    ...
    let outcome: std::result::Result<(), WmWindowError> = db
        .with_connection(|conn| Ok(save_batch(conn, device, &[window])))
        .expect("with_connection succeeds");
    assert!(matches!(outcome, Err(WmWindowError::GeometryOutOfBounds)));
}

#[test]
fn save_batch_rejects_an_unknown_workspace() {
    let (_dir, db, device) = open_test_vault("wm-windows-unknown-workspace");
    let window = one_tab_window(Uuid::new_v4(), 1);

    let outcome: std::result::Result<(), WmWindowError> = db
        .with_connection(|conn| Ok(save_batch(conn, device, &[window])))
        .expect("with_connection succeeds");
    assert!(matches!(outcome, Err(WmWindowError::UnknownWorkspace)));
}

#[test]
fn save_batch_enforces_the_device_window_cap() {
    let (_dir, db, device) = open_test_vault("wm-windows-cap");
    let workspace = db
        .with_connection(|conn| Ok(create_workspace(conn, device)?))
        .expect("create workspace");
    ...
    let outcome: std::result::Result<(), WmWindowError> = db
        ...
        .expect("with_connection succeeds");
    assert!(matches!(outcome, Err(WmWindowError::TooManyWindows)));
}

#[test]
fn close_windows_removes_windows_and_their_tabs_and_ignores_unknown_ids() {
    let (_dir, db, device) = open_test_vault("wm-windows-close");
    let workspace = db
        .with_connection(|conn| Ok(create_workspace(conn, device)?))
        .expect("create workspace");
    let window = one_tab_window(workspace.workspace_id, 1);
...
fn load_all_orders_windows_by_stack_order_and_skips_ones_without_tabs() {
    let (_dir, db, device) = open_test_vault("wm-windows-load-order");
    let workspace = db
        .with_connection(|conn| Ok(create_workspace(conn, device)?))
        .expect("create workspace");
