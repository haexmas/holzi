...
//! `shell_save_windows` / `shell_close_windows`).
//!
...
fn validate_shape(windows: &[WindowWrite]) -> std::result::Result<(), ShellWindowError> {
    let mut seen_tab_ids = HashSet::new();
    for w in windows {
        if w.tabs.len() < MIN_TABS_PER_WINDOW || w.tabs.len() > MAX_TABS_PER_WINDOW {
            return Err(ShellWindowError::TabCountOutOfRange);
        }
        if !w.tabs.iter().any(|t| t.tab_id == w.active_tab_id) {
            return Err(ShellWindowError::ActiveTabNotInWindow);
        }
        ...
        if !(x_ok && y_ok && width_ok && height_ok) {
            return Err(ShellWindowError::GeometryOutOfBounds);
        }
        for t in &w.tabs {
            ...
            if !app_id_ok {
                return Err(ShellWindowError::InvalidAppId);
            }
            if !seen_tab_ids.insert(t.tab_id) {
                return Err(ShellWindowError::DuplicateTabId);
            }
        }
    }
    Ok(())
...
/// all-or-nothing (contract `shell_save_windows`). Validates I4 (workspace
...
pub fn save_batch(
    ...
    windows: &[WindowWrite],
) -> std::result::Result<(), ShellWindowError> {
    ...
    for w in windows {
        ...
        if workspace_exists == 0 {
            return Err(ShellWindowError::UnknownWorkspace);
        }
    }
    ...
    if current_total + incoming_new > MAX_WINDOWS_PER_DEVICE {
        return Err(ShellWindowError::TooManyWindows);
    }
...
/// `stack_order` ascending (contract `shell_load_layout`'s
