...
import type { ShellTabApi } from '~/composables/useShellTab'
...
/**
 * The chat's side of the Shell (spec 020-tab-navigation, T050): tab history
 ...
 */
export function useChatShell(deps: {
  ...
  const { shellTab, state, runAction } = deps
  ...
  async function respondToApproval(
    ...
    try {
      ...
      if (state.pendingApprovals.value.length === 0) shellTab.clearAttention()
    } catch (e: unknown) {
  ...
  ) => shellTab.registerActionHandler(id, ({ input }) => handler(input))
