...
/**
 * Wire types for the window manager layout commands (spec 015-workspace-shell,
 ...
 * generated `WorkspaceDto`/`TabDto`/`WindowDto`/`WmLayoutDto`/
...
/**
 * Owns the persistence side of the open workspaces, windows and tabs: a serialized invoke queue (the
 * contract's "at most one window manager call in flight" rule — otherwise a delayed
 * `wm_save_windows` could resurrect a window a later `wm_close_windows`
 * already removed), a 400ms debounce that batches geometry/stack changes
 * into one `wm_save_windows`, and dirty-retry (a window that failed to
 ...
 * Translating between the store's `WmWindow`/`WmTab` and these DTOs
 * is the caller's job too (`stores/windowManager.ts`, T048) — this composable only
 ...
 * (unlike `useWmCloseConfirm`'s `pending` ref) — there is exactly one
 * saved set of windows and tabs per device, so `stores/windowManager.ts` calls this once at setup.
 *
 * `invokeFn` defaults to the real Tauri `invoke` — `scripts/check-wm-
 ...
 */
export function useWmLayout(invokeFn: WmInvokeFn = tauriInvoke) {
  ...
  async function flushPending(): Promise<void> {
    if (pendingSaves.size > 0) {
      ...
      try {
        await invokeFn('wm_save_windows', { args: { windows } })
        for (const w of windows) pendingSaves.delete(w.windowId)
      } catch (error) {
        console.error(
          '[wm] wm_save_windows failed; windows stay dirty for the next save',
          error,
        )
      }
    }
    if (pendingCloses.size > 0) {
      ...
      try {
        await invokeFn('wm_close_windows', { args: { windowIds } })
        for (const id of windowIds) pendingCloses.delete(id)
      } catch (error) {
        console.error(
          '[wm] wm_close_windows failed; windows stay pending for the next save',
          error,
        )
      }
    }
  ...
  function loadLayout(): Promise<WmLayoutDto> {
    return runExclusive(
      async () => (await invokeFn('wm_load_layout')) as WmLayoutDto,
    )
  }
  ...
  function createWorkspace(): Promise<WorkspaceDto> {
    return runExclusive(
      async () => (await invokeFn('wm_create_workspace')) as WorkspaceDto,
    )
  }
  ...
  function deleteWorkspace(
    ...
    return runExclusive(
      async () =>
        (await invokeFn('wm_delete_workspace', {
          args: { workspaceId },
  ...
  function setActiveWorkspace(workspaceId: string): Promise<void> {
    return runExclusive(() =>
      invokeFn('wm_set_active_workspace', { args: { workspaceId } }).then(
        () => undefined,
      ),
    )
  }
