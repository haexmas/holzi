...
const rows = computed(() =>
  shell.workspaces.map((workspace) => ({
    ...
    windowCount: shell.windows.filter((w) => w.workspaceId === workspace.id)
      .length,
    hasAttention: shell.workspaceHasAttention(workspace.id),
  })),
)

const currentWorkspaceWindows = computed(() =>
  shell.windows.filter((w) => w.workspaceId === shell.activeWorkspaceId),
)
...
<template>
  <UiDrawerModal
    :open="open"
    :title="t('shell.workspaces.title')"
    ...
    <template #content>
      <div class="flex flex-col gap-2 overflow-x-hidden p-4">
        <div
          ...
          :class="
            row.workspace.id === shell.activeWorkspaceId
